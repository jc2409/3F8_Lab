...
With learning rates chosen according to the spectral radius of each
feature matrix, $l=1$ gives the best test performance (LL $\approx-0.241$),
while $l=0.01$ fails to converge and $l=0.1$ offers a good intermediate
result (LL $\approx-0.419$).
\end{abstract}
...
\begin{figure}
\begin{centering}
\includegraphics[width=0.3\paperwidth]{fig_lin_ll_train}\hspace{1cm}\includegraphics[width=0.3\paperwidth]{fig_lin_ll_test}
\par\end{centering}
...
were set to $\eta=0.01/800$, $0.1/800$, $0.04/800$ respectively,
with 1000, 1000, and 5000 gradient steps. The contour plots are shown
...
shows no meaningful decision boundary, and the learning curves
(Figure~\ref{fig:rbf_learning_curves}, top row) remain nearly flat
throughout. For $l=0.1$ (middle), the boundary is smooth and curved,
fitting the shape of the class overlap better than a straight line,
and both log-likelihood curves converge steadily. For $l=1$ (right),
the broad basis functions overlap heavily, so the boundary is smoother
but still noticeably non-linear; the training LL converges to a higher
value than the test LL, hinting at mild overfitting.

\begin{figure}
\begin{centering}
\includegraphics[width=0.32\textwidth]{fig_rbf001_contour}\hspace{0.15cm}\includegraphics[width=0.32\textwidth]{fig_rbf01_contour}\hspace{0.15cm}\includegraphics[width=0.32\textwidth]{fig_rbf1_contour}
\par\end{centering}
...
\begin{figure}
\begin{centering}
\includegraphics[width=0.3\textwidth]{fig_rbf001_ll_train}\hspace{0.15cm}\includegraphics[width=0.3\textwidth]{fig_rbf001_ll_test}\\[0.3cm]
\includegraphics[width=0.3\textwidth]{fig_rbf01_ll_train}\hspace{0.15cm}\includegraphics[width=0.3\textwidth]{fig_rbf01_ll_test}\\[0.3cm]
\includegraphics[width=0.3\textwidth]{fig_rbf1_ll_train}\hspace{0.15cm}\includegraphics[width=0.3\textwidth]{fig_rbf1_ll_test}
\par\end{centering}
\caption{Training (left) and test (right) average log-likelihood curves for
RBF classifiers with $l=0.01$ (top), $l=0.1$ (middle), $l=1$ (bottom).\label{fig:rbf_learning_curves}}
\end{figure}
...
The $l=0.1$ model gives a test LL of $-0.419$, a clear improvement
over the linear baseline. The $l=1$ model achieves the best performance
overall: train LL $-0.222$ and test LL $-0.241$, with a train--test
gap of only $0.019$, indicating the model generalises well without
overfitting.
...
For $l=1$, Class~1 accuracy rises to 98\% while Class~0 accuracy
sits at 85\% --- the model is somewhat biased toward predicting Class~1,
though both classes are classified more accurately than the linear model. Overall,
$l=1$ with the corrected learning rate gives the best test
log-likelihood and accuracy among all models tested.
...
\begin{tabular}{cc|c|c}
 ...
 &  & 0 & 1\tabularnewline
\cline{2-4}
$y$ & 0 & $0.8545$ & $0.1455$\tabularnewline
\cline{2-4}
...
and a suitable learning rate both matter. With $l=0.01$ the features
are too sparse for gradient ascent to make progress. With $l=0.1$
we get a test LL of $-0.419$ and balanced accuracy ($\sim$82\% on
both classes). With $l=1$, using a learning rate informed by the
spectral radius of the feature matrix ($\eta=0.04/800$, 5000 steps),
the model achieves the best test LL ($-0.241$) and generalises well
(train--test gap of only $0.019$). This highlights that an
inappropriate learning rate can mask a model's true capacity:
the original $\eta=0.5/800$ caused oscillations and poor convergence
for $l=1$, while the corrected rate reveals it as the strongest model.
In practice, both $l$ and $\eta$ should be chosen carefully.
